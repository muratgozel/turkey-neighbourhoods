...
export class TurkeyNeighbourhoods extends Resource {
    ...
    async dist() {
        ...
        const cityNames = [...new Set(raw.map((arr: string[]) => arr[1]))]
        ...
        const cityCodes = [...new Set(raw.map((arr: string[]) => arr[0]))]
        ...
        const cityList = raw.reduce((memo: {code: string | undefined, name: string | undefined}[], arr: string[]) => {
        ...
        const cityMapCodeName = cityList.reduce((memo: {[index: string]: string | undefined}, obj: {code: string | undefined, name: string | undefined}) => {
        ...
        const cityMapCodeDistricts = raw.reduce((memo: {[index: string]: string[]}, arr: string[]) => {
        ...
        const postalCodes = raw.map((arr: string[]) => arr[4])
        ...
        const mapCodeDistrictNeighbourhoods = raw.reduce((memo: {[index: string]: {[index: string]: string[]}}, arr: string[]) => {
            if (typeof arr[0] === 'string' && typeof arr[2] === 'string') {
    ...
    async output(xlsxpath: string) {
        ...
        const json: {il: string, 'ilçe': string, semt_bucak_belde: string, Mahalle: string, PK: string}[] = typeof worksheet !== 'undefined' ? XLSX.utils.sheet_to_json(worksheet) : []
    ...
    formatOutput(json: {il: string, 'ilçe': string, semt_bucak_belde: string, Mahalle: string, PK: string}[]) {
        return json.map(obj => {
            return [
                obj['PK'].slice(0, 2),
